...
class Mixture(SymbolicDistribution):
    ...
    def rv_op(cls, weights, *components, size=None, rngs=None):
        ...
        if rngs is not None:
            ...
            *_, mix_indexes_rng = rngs
            ...
            # Create new rng for the choices internal RV
        ...
        component_ndim_batch = components[0].ndim - component_ndim_supp
        if single_component:
            # One dimension is taken by the mixture axis in the single component case
            component_ndim_batch -= 1
        weights_ndim_reps = max(0, weights.ndim - component_ndim_batch - 1)
        ...
        if weights_ndim_reps:
            warnings.warn(
                "Weights have higher dimensionality than what is implied by the Mixture "
                "components or Mixture size.\n Components will be automatically resized "
                "to match the dimensionality of the weights.",
                UserWarning,
            )
            new_size = at.concatenate(
                [
                    weights.shape[:weights_ndim_reps],
                    components[0].shape[:component_ndim_batch],
                ]
            )
        ...
        # Broadcast weights to (*batched dimensions, stack dimension), ignoring support
        # dimensions
        if single_component:
            weights_broadcast_shape_ = components_[0].shape[: component_ndim_batch + 1]
        else:
            weights_broadcast_shape_ = at.concatenate(
                [
                    components_[0].shape[:component_ndim_batch],
                    [len(components_)],
                ],
                axis=-1,
            )
        ...
        if single_component:
            mix_out_ = at.take_along_axis(
                components_[0], mix_indexes_padded, axis=-component_ndim_supp - 1
            )
            # There is a Aeasara bug in squeeze with negative axis
            # mix_out_ = at.squeeze(mix_out_, axis=-component_ndim_supp - 1)
            mix_out_ = at.squeeze(mix_out_, axis=mix_out_.ndim - component_ndim_supp - 1)
        else:
            stacked_components_ = at.stack(components_, axis=-1)
            mix_out_ = at.take_along_axis(stacked_components_, mix_indexes_padded, axis=-1)
            # There is a Aeasara bug in squeeze with negative axis
    ...
    def _resize_components(cls, size, *components):
        # Single component
        if len(components) == 1:
            # We have to keep the length of the mixture axis intact, because that's what
            # determines the number of mixture components
            component_mix_axis = -components[0].owner.op.ndim_supp - 1
            axis_size = components[0].shape[component_mix_axis]
            size = tuple(size) + (axis_size,)

            component_node = components[0].owner
            rng, old_size, dtype, *dist_params = component_node.inputs
            new_component = component_node.op.make_node(
                rng, size, dtype, *dist_params
            ).default_output()
            ...
            for component in components:
                ...
                rng, old_size, dtype, *dist_params = component_node.inputs
                new_components.append(
                    component_node.op.make_node(rng, size, dtype, *dist_params).default_output()
                )
            return new_components
    ...
    def change_size(cls, rv, new_size, expand=False):
        ...
        if expand:
            ...
            if len(rv.tag.components) == 1:
                # New size should ignore the mixture axis dimension
                component_mix_axis = -component.owner.op.ndim_supp - 1
                new_size = to_tuple(new_size) + tuple(component.shape[:component_mix_axis])
            else:
                size_dims = component.ndim - component.owner.op.ndim_supp
                new_size = to_tuple(new_size) + tuple(components[0].shape[:size_dims])
...
def marginal_mixture_logprob(op, values, rng, weights, *components, **kwargs):
    ...
    # single component
    if len(components) == 1:
        components_logp = logp(components[0], value[..., None])
    else:
